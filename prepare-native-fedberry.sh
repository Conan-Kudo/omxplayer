...
check_rpm_installed() {
	echo -n "."
	if [ $(rpm -q $1 2> /dev/null) -eq 0 ];
	then
		MISSING_PKGS="$MISSING_PKGS $1"
	fi
}

echo "Modifying for native build on Fedora"

if [ -z `which sudo` ] ; then
    dnf install -y sudo
fi

echo "Checking rpm database for missing packages"
REQUIRED_PKGS="ca-certificates git-core subversion binutils alsa-lib-devel libva pcre-devel libidn-devel boost-devel freetype-devel libusbx-devel dbus-devel openssl-devel libssh-devel libsmbclient-devel gcc gcc-c++ sed pkgconfig"
MISSING_PKGS=""
...
if [ ! -z "$MISSING_PKGS" ]; then
	...
	echo "Run sudo dnf install $MISSING_PKGS"
	exit 1
else
...
CFLAGS += -O2 -g -march=armv7-a -mfloat-abi=hard -mfpu=neon `pkg-config dbus-1 --cflags`

BUILDROOT	:=/home/ngompa/Downloads/omxplayer
TOOLCHAIN	:=/usr/
LD			:= \$(TOOLCHAIN)/bin/ld
...
LDFLAGS		+= -L/usr/lib/vc -L/usr/lib -lfreetype
INCLUDES	+= -I/usr/include/vc/interface/vcos/pthreads \
			-I/usr/include/vc \
			-I/usr/include/vc/interface/vmcs_host \
			-I/usr/include/vc/interface/vmcs_host/linux \
...
REQUIRED_PKGS="raspberrypi-vc-libs-devel"
MISSING_PKGS=""
for pkg in $REQUIRED_PKGS
do
	check_rpm_installed $pkg
